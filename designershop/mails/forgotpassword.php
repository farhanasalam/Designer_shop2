...
if (isset($_POST['submits'])) {
	$email=$_POST['email'];
	$sqs="SELECT *   FROM login WHERE email='$email'";
	$result=mysqli_query($conn,$sqs);
	$row=mysqli_fetch_assoc($result);
    $a=$row['email'];
    $b=$row['log_id'];
  //   $lidres="SELECT * FROM kalolsavam_tb1_login WHERE l_id='$b'";
...
$mail->addAddress($row["email"]);
  ...
  else
  {
   ...
   //echo "Message has been sent successfully";
         ...
         echo "<script>alert('Please provide valid informations');window.location='forgotpassword.php';</script>";
     }
